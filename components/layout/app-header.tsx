...
export function AppHeader() {
  ...
  return (
    <header className="sticky top-0 z-30 border-b border-border/90 bg-card/85 backdrop-blur-md">
      <div className="mx-auto flex h-14 w-full max-w-6xl items-center justify-between px-4">
        {/* Logo */}
        <Link href="/" className="group flex items-center gap-2">
          <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-primary text-xs font-bold text-white shadow-sm transition group-hover:bg-primary-hover">
            K
          </span>
          <span className="font-serif text-sm font-semibold tracking-tight text-foreground">
            知识图谱<span className="text-primary">笔记</span>
          </span>
        </Link>
        ...
        <div className="flex items-center gap-1">
          ...
          {user ? (
              ...
              {menuOpen && (
                <div className="absolute right-0 top-full mt-1.5 w-48 overflow-hidden rounded-xl border border-border bg-card shadow-lg">
                  ...
                  <div className="p-1">
                    ...
                    >
                      <svg className="h-4 w-4 text-muted-foreground" viewBox="0 0 16 16" fill="none">
            ...
            <>
              <Link
                href="/login"
                className="ml-2 rounded-lg px-3 py-1.5 text-sm font-medium text-muted-foreground transition hover:bg-muted hover:text-foreground"
              >
                登录
              </Link>
              <Link
                href="/register"
                className="ml-1 rounded-lg bg-primary px-3 py-1.5 text-sm font-medium text-white shadow-sm transition hover:bg-primary-hover"
              >
                注册
              </Link>
            </>
